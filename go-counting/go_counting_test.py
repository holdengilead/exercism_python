...
class GoCountingTest(unittest.TestCase):
    def test_black_corner_territory_on_5x5_board(self) -> None:
    ...
    def test_invalid_because_x_is_too_high_for_5x5_board(self) -> None:
        board = Board(["  B  ", " B B ", "B W B", " W W ", "  W  "])
        with self.assertRaises(ValueError) as err:
    ...
    def test_invalid_because_y_is_too_low_for_5x5_board(self) -> None:
        board = Board(["  B  ", " B B ", "B W B", " W W ", "  W  "])
        with self.assertRaises(ValueError) as err:
            board.territory(x=1, y=-1)
        self.assertEqual(type(err.exception), ValueError)
    ...
    def test_invalid_because_y_is_too_high_for_5x5_board(self) -> None:
        board = Board(["  B  ", " B B ", "B W B", " W W ", "  W  "])
        with self.assertRaises(ValueError) as err:
    ...
    def test_one_territory_is_the_whole_board(self) -> None:
        board = Board([" "])
    ...
    def test_two_territory_rectangular_board(self) -> None:
        board = Board([" BW ", " BW "])
        territories = board.territories()
    ...
    def test_two_region_rectangular_board(self) -> None:
        board = Board([" B "])
        territories = board.territories()
    ...
    # Utility functions
    def assertRaisesWithMessage(self, exception):  # type: ignore
