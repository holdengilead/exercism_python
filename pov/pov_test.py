...
class PovTest(unittest.TestCase):
    def test_results_in_the_same_tree_if_the_input_tree_is_a_singleton(self) -> None:
        tree = Tree("x")
        expected = Tree("x")
        self.assertTreeEquals(tree.from_pov("x"), expected)

    def test_can_reroot_a_tree_with_a_parent_and_one_sibling(self) -> None:
        tree = Tree("parent", [Tree("x"), Tree("sibling")])
        expected = Tree("x", [Tree("parent", [Tree("sibling")])])
        self.assertTreeEquals(tree.from_pov("x"), expected)

    def test_can_reroot_a_tree_with_a_parent_and_many_siblings(self) -> None:
        tree = Tree("parent", [Tree("a"), Tree("x"), Tree("b"), Tree("c")])
        expected = Tree("x", [Tree("parent", [Tree("a"), Tree("b"), Tree("c")])])
        self.assertTreeEquals(tree.from_pov("x"), expected)

    def test_can_reroot_a_tree_with_new_root_deeply_nested_in_tree(self) -> None:
        tree = Tree(
            "level-0",
    ...
    def test_can_reroot_a_complex_tree_with_cousins(self) -> None:
        tree = Tree(
            "grandparent",
            [
                Tree(
    ...
    def test_errors_if_target_does_not_exist_in_a_singleton_tree(self) -> None:
        tree = Tree("x")
    ...
    def test_errors_if_target_does_not_exist_in_a_large_tree(self) -> None:
        tree = Tree(
            "parent",
            [
                Tree("x", [Tree("kid-0"), Tree("kid-1")]),
    ...
    def test_can_find_path_to_parent(self) -> None:
        tree = Tree("parent", [Tree("x"), Tree("sibling")])
        expected = ["x", "parent"]
        self.assertEqual(tree.path_to("x", "parent"), expected)

    def test_can_find_path_to_sibling(self) -> None:
        tree = Tree("parent", [Tree("a"), Tree("x"), Tree("b"), Tree("c")])
        expected = ["x", "parent", "b"]
        self.assertEqual(tree.path_to("x", "b"), expected)

    def test_can_find_path_to_cousin(self) -> None:
        tree = Tree(
            "grandparent",
            [
                Tree(
    ...
    def test_can_find_path_not_involving_root(self) -> None:
        tree = Tree(
            "grandparent",
    ...
    def test_can_find_path_from_nodes_other_than_x(self) -> None:
        tree = Tree("parent", [Tree("a"), Tree("x"), Tree("b"), Tree("c")])
        expected = ["a", "parent", "c"]
        self.assertEqual(tree.path_to("a", "c"), expected)

    def test_errors_if_destination_does_not_exist(self) -> None:
        tree = Tree(
            "parent",
            [
                Tree("x", [Tree("kid-0"), Tree("kid-1")]),
    ...
    def test_errors_if_source_does_not_exist(self) -> None:
        tree = Tree(
            "parent",
            [
                Tree("x", [Tree("kid-0"), Tree("kid-1")]),
    ...
    def assertTreeEquals(self, result, expected) -> None:
