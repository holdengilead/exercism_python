...
class ReactTest(unittest.TestCase):
    def test_input_cells_have_a_value(self) -> None:
        input = InputCell(10)
        self.assertEqual(input.value, 10)

    def test_an_input_cell_s_value_can_be_set(self) -> None:
        input = InputCell(4)
        input.value = 20
        self.assertEqual(input.value, 20)

    def test_compute_cells_calculate_initial_value(self) -> None:
        input = InputCell(1)
        output = ComputeCell(
            [
                input,
    ...
    def test_compute_cells_take_inputs_in_the_right_order(self) -> None:
        one = InputCell(1)
        two = InputCell(2)
        output = ComputeCell(
            [
    ...
    def test_compute_cells_update_value_when_dependencies_are_changed(self) -> None:
        input = InputCell(1)
        output = ComputeCell(
            [
                input,
    ...
    def test_compute_cells_can_depend_on_other_compute_cells(self) -> None:
        input = InputCell(1)
    ...
    def test_compute_cells_fire_callbacks(self) -> None:
        input = InputCell(1)
        output = ComputeCell(
            [
                input,
    ...
    def test_callbacks_do_not_report_already_reported_values(self) -> None:
        input = InputCell(1)
        output = ComputeCell(
            [
                input,
    ...
    def test_callbacks_can_fire_from_multiple_cells(self) -> None:
        input = InputCell(1)
    ...
    def test_callbacks_can_be_added_and_removed(self) -> None:
        input = InputCell(11)
        output = ComputeCell(
            [
                input,
    ...
    # Utility functions.
    def callback_factory(self, observer: list[int]) -> Callable[[int], None]:
        def callback(observer: list[int], value: int) -> None:
